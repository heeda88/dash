...
#     parser.add_argument(
#         '--manifest','-m' ,default='/data/namu/speech/한국어_음성_분야/manifest.json' ,help='path to JSON manifest file',
#     )
...
# args = parse_args()
args_manifest='/data/namu/kospeech/한국어_음성_분야/convert/manifest.json'
print('Loading data...')
...
data, wer, cer, wmr, mwa, num_hours, vocabulary, alphabet, metrics_available = load_data(args_manifest)
...
layout_nav = [
        dbc.NavbarSimple(
            children=[
                dbc.NavItem(dbc.NavLink('Statistics', id='stats_link', href='/dash/speech', active=True)),
                dbc.NavItem(dbc.NavLink('Samples', id='samples_link', href='/dash/speech/sample')),
            ],
...
@app.callback(
    [Output('stats_link', 'active'), Output('samples_link', 'active')],
    [Input('url', 'pathname')],
)
def nav_click(url):
    if url == '/dash/speech/sample':
        return [False, True]
    elif url == '/dash/speech':
        return [True, False]
